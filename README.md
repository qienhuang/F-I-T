...
[[中文/Chinese]](docs/zh_cn/README.zh_cn.md)
...
**Current version:** [v2.4 (academicized draft)](docs/v2.4.md)  
...
- **Current spec (v2.4, EST + Tier-1 validation)**: [docs/v2.4.md](docs/v2.4.md) (EN), [docs/zh_cn/v2.4.zh_cn.md](docs/zh_cn/v2.4.zh_cn.md) (中文/Chinese)  
...
>### F-I-T (Force-Information-Time) System Evolution Meta-Framework v1.0 （original intuition)
...
To set realistic expectations, we explicitly state what FIT v2.4 does **not** claim:
...
For claims involving thresholds or critical points (e.g., "transition at $C = C^*$ "), monotone rescaling is insufficient. We require **metric stability**.
...
- Fixed point: $x^* = 0$
- Force: $F(t) := x(t) - x^*## 4.4 Solvable Example: Linear Relaxation System (continued)
...
**Formal**: $\exists$ multiple fixed points $\{S^*_i\}$ with:
- Each has basin $B_i$
- $C(S^*_i) \approx C_{\max}$ locally
...
**Concrete example**: GPT-style model with safety fine-tuning
...
- Monitor: If $\sigma^2(F)$ increases → additional safety training needed
...
- **Current core spec (v2.4, EST + Tier-1 validation)**:  
  `docs/v2.4.md` (EN), `docs/zh_cn/v2.4.zh_cn.md` (中文)
- **Recommended entry point**:  
  Start with `docs/v2.4.md`. For a legacy discussion-first version, see `docs/v2.1.md`.
...
- [ ] Freeze `docs/v2.4.md` as the authoritative 2.x reference (discrete, classical) and tag a `v2.4` release.  
- [ ] Keep `docs/v2.3.md` (previous edition) and `docs/v2.1.md` (legacy discussion edition) as stable historical references.  
- [ ] Finalize the **proposition registry schema** (YAML/JSON) for P1-P18:
  - IDs, short names, scope conditions, estimators, boundary declarations, protocol, status.
...
- [ ] Add a short “How to falsify FIT” guide to the repo (where to attack first, and how to record negative results).
...
- [ ] Draft a `fit_continuous_v3.0-alpha.md` describing:
...
- [ ] Draft a `fit_quantum_v3.0-alpha.md` or a short paper:
...
- `docs/v2.4.md` - current spec (EST + Tier-1 computational validation)
- `docs/v2.3.md` - previous edition (Estimator Specification Layer)
- `docs/v2.1.md` - legacy discussion edition
- `docs/v1.0.md` - the intuition-first overview
- `docs/zh_cn/` - Chinese documents (including v2.4 translation and archived v2.0)
...
If you want to contribute, the highest-leverage path is to try to **break v2.4** (or the Tier-1 claims in v2.3):
...
Please cite **v2.4** (recommended) via `CITATION.cff` or the Zenodo DOI above.

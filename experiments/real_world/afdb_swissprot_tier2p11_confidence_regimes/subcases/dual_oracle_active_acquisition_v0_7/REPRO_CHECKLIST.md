# REPRO_CHECKLIST — Dual‑Oracle + Joint‑Gap Allocation (v0.7)
...
- `allocation_trace.csv` exists and contains per (policy, round):
  - `q_pae`, `q_msa`, `gap_pae`, `gap_msa`
  - `tpr_val_pae`, `tpr_val_msa` (policy diagnostics)
- Holdout metrics are not used for acquisition decisions (check `policy_diag_split` in snapshots)

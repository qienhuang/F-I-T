...
echo "[5/6] aggregate v2.6 (bilevel CVaR/expected shortfall gate + tail quantile audit)"
python "${CASE_DIR}/scripts/aggregate_v2_6.py" \
  --main_run "${CASE_DIR}/out_smoke/MAIN" \
  --baseline_root "${CASE_DIR}/out_smoke_baseline" \
  --policy_root "${CASE_DIR}/out_smoke_policy"
...
echo "CI-style check PASSED (v2.6)."

...
def main() -> None:
    ...
    templates = {
        "allowed_phrase": "Under the preregistered protocol and cost-aware low-FPR cap, {policy} robustly exceeds the random baseline band under the bilevel CVaR (expected shortfall) gate (CI strictly positive).",
        "forbidden_phrase": "Under the preregistered protocol, {policy} outperforms random.",
        "safe_phrase": "Under the preregistered protocol and cost-aware low-FPR cap, {policy} does not show a statistically robust advantage over the random baseline band under the bilevel CVaR (expected shortfall) gate (CI crosses zero or is undefined).",

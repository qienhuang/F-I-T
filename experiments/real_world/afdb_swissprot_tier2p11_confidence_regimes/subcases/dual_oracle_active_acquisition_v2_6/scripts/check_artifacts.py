...
def main() -> None:
    ...
    gate_alpha = summary.get("cvar_gate_alpha", None)
    ...
    if gate_alpha is None:
        raise SystemExit("bilevel_bootstrap_summary.json missing cvar_gate_alpha.")

    gate = json.loads((rd / "claims_gate_report.json").read_text(encoding="utf-8"))
    if gate.get("mode") != "bilevel_bootstrap_cvar_gate":
        raise SystemExit("claims_gate_report.json mode != bilevel_bootstrap_cvar_gate (v2.6 expects CVaR gate).")
    if gate.get("gate_metric") != "bilevel_cvar_gate_margin_ci_low":
        raise SystemExit("claims_gate_report.json gate_metric != bilevel_cvar_gate_margin_ci_low.")

    bi = pd.read_csv(rd / "frontier_bilevel_robust_table.csv")
    base_need = {
        "bilevel_cvar_gate_alpha",
        "bilevel_cvar_gate_margin_ci_low",
        "bilevel_cvar_gate_margin_ci_high",
    ...
    if not base_need.issubset(set(bi.columns)):
        raise SystemExit("frontier_bilevel_robust_table.csv missing v2.6 base gate columns.")
    ...
    # Gate consistency: allowed => cvar gate ci low > 0
    bad = []
    for pol, g in (gate.get("by_policy", {}) or {}).items():
        lo = g.get("bilevel_cvar_gate_margin_ci_low", None)
        allowed = bool(g.get("allowed", False))
        if lo is not None and allowed and float(lo) <= 0:
            bad.append(pol)
    if bad:
        raise SystemExit("Claims gate inconsistency (allowed but CVaR gate ci_low<=0):\n" + "\n".join(f"- {p}" for p in bad))

    print("OK: v2.6 artifacts + leakage pass + bilevel CVaR gate + tail/CVaR audit columns + claims pack present.")

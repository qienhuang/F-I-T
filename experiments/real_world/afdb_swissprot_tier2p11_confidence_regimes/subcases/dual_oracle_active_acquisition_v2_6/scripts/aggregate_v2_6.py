...
def _cvar(margins: np.ndarray, alpha: float) -> float:
    # Expected shortfall: mean of worst alpha-fraction of margins
...
def _bilevel_bootstrap_stats(
    ...
    Bilevel bootstrap producing:
      ...
      - tail quantiles + min-over-q (audit)
      - CVaR / expected shortfall for each alpha (audit)
      - CVaR at gate alpha (primary gate statistic)
...
def main() -> None:
    ...
    for p in policies:
        ...
        allow_cvar = bool(np.isfinite(stats["cvar_gate_margin_ci_low"]) and stats["cvar_gate_margin_ci_low"] > 0)
        ...
        row = {
            ...
            "gate_metric": "bilevel_cvar_gate_margin_ci_low",
            "claim_outperforms_random_allowed": allow_cvar,
        }
    ...
    df_bi = pd.DataFrame(rows_bi).sort_values(by=["claim_outperforms_random_allowed","bilevel_cvar_gate_margin_mean"], ascending=[False, False])
    df_bi.to_csv(main_run / "frontier_bilevel_robust_table.csv", index=False)

    gate = {
        "mode": "bilevel_bootstrap_cvar_gate",
        "rule": "allow 'outperforms random' only if bilevel_cvar_gate_margin_ci_low > 0 (expected shortfall at preregistered gate alpha)",
        "gate_metric": "bilevel_cvar_gate_margin_ci_low",
    ...
    write_json(main_run / "bilevel_bootstrap_summary.json", {
        ...
        "tail_quantiles": tail_qs,
        "tail_secondary_statistic": str(bb.get("statistic_secondary", "tail_margin_min_over_q")),

        "cvar_alphas": cvar_alphas,
        "cvar_gate_alpha": float(cvar_gate_alpha),
        "primary_statistic": str(bb.get("statistic_primary", "cvar_gate_alpha_expected_shortfall")),
        "gate_metric": "bilevel_cvar_gate_margin_ci_low",
    ...
    print("v2.6 aggregation complete:")
    for f in [
        "baseline_band.json",
        "policy_band.json",

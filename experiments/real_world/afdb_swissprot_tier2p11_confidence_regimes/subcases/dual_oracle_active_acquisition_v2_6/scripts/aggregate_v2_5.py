...
def _q_tag(q: float) -> str:
    # 0.05 -> q05, 0.1 -> q10, 0.2 -> q20, 0.125 -> q12p5
    x = float(q) * 100.0
    if abs(x - round(x)) < 1e-9:
        return f"q{int(round(x)):02d}"
    # one decimal place for non-integer percent
    return f"q{str(x).replace('.','p')}"


def _bilevel_bootstrap_sweep(
    cov_vec: np.ndarray,
    cost_vec: np.ndarray,
    ...
    """
    Bilevel bootstrap with quantile sweep.

    Each replicate:
      - resample baseline seeds
      - resample policy seeds
      - recompute baseline upper at each policy-seed cost under resampled baseline
      - margins = cov - upper
      - compute mean(margins), quantile_q(margins) for each q in tail_qs
      - compute min_over_q(quantile_q(margins)) as a stability statistic

    Returns point + CI for mean, each tail q, and the min-over-q statistic.
    """
    ...
    if n_policy == 0 or n_base == 0 or not tail_qs:
        out = {
    ...
    for _ in range(int(n_boot)):
        ...
        m_min = float(np.min(tails))
        min_stats.append(m_min)
        if m_min > 0:
            min_adv += 1

    mean_stats = np.asarray(mean_stats, dtype=float)
    min_stats = np.asarray(min_stats, dtype=float)
    ...
    out = {
        ...
        "tail_min_margin_mean": float(np.mean(min_stats)),
        "tail_min_margin_ci_low": float(np.quantile(min_stats, (1-ci)/2)),
        "tail_min_margin_ci_high": float(np.quantile(min_stats, 1-(1-ci)/2)),
        "tail_min_p_advantage": float(min_adv / max(1, len(min_stats))),
    }
...
def main() -> None:
    ...
    tail_qs = sorted(list(dict.fromkeys(tail_qs)))  # unique, ordered
    ...
    for p in policies:
        ...
        stats = _bilevel_bootstrap_sweep(
            cov_vec=covs,
            cost_vec=costs,
        ...
        allow_min = bool(np.isfinite(stats["tail_min_margin_ci_low"]) and stats["tail_min_margin_ci_low"] > 0)
        ...
        row = {
            ...
            "gate_metric": "bilevel_tail_min_margin_ci_low",
            "claim_outperforms_random_allowed": allow_min,
        }
    ...
    df_bi = pd.DataFrame(rows_bi).sort_values(by=["claim_outperforms_random_allowed","bilevel_tail_min_margin_mean"], ascending=[False, False])
    df_bi.to_csv(main_run / "frontier_bilevel_robust_table.csv", index=False)

    gate = {
        "mode": "bilevel_bootstrap_tail_sweep",
        "rule": "allow 'outperforms random' only if bilevel_tail_min_margin_ci_low > 0 (min over preregistered tail quantiles)",
        "gate_metric": "bilevel_tail_min_margin_ci_low",
    ...
    write_json(main_run / "bilevel_bootstrap_summary.json", {
        ...
        "tail_quantiles": tail_qs,
        "primary_statistic": str(bb.get("statistic_primary", "tail_margin_min_over_q")),
    ...
    print("v2.5 aggregation complete:")
    for f in [
        "baseline_band.json",
        "policy_band.json",

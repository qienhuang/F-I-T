...
from .plot_onepager import plot_onepage_v0_4
...
def main() -> None:
    ...
    # Holdout/pool per oracle availability (here intersection by dataset filtering)
    ...
    ranking_cfg = cfg["acquisition"]["selection"]["within_oracle_ranking"]
    ...
    for policy in policies:
        ...
        for r in range(0, rounds + 1):
            ...
            y_c3_val = c3_msa[lv_msa.val] if lv_msa and len(lv_msa.val) else c3_msa
            ...
            # Determine remaining budgets/pools
            ...
            if policy == "fixed_split":
                ...
                elif policy == "adaptive_need_based_joint":
                    # Prioritize the oracle whose alarm is not yet usable at primary cap
                    if pae_ok and (not msa_ok):
                        q_pae, q_msa = 0, q_round_total
                    elif msa_ok and (not pae_ok):
            ...
            # select ids per oracle
            def select_ids(unl_set: set[str], oracle: str, q: int, model, y_lab: np.ndarray):
                unl_list = sorted(list(unl_set))
                if q <= 0 or len(unl_list) == 0:
                    return [], np.array([], dtype=float), unl_list
                rank_mode = str(ranking_cfg.get(oracle, "uncertainty")) if model.trained else fallback_no_model
            ...
            sel_pae, probs_pae, unl_pae_list = select_ids(unlabeled_pae, "PAE", q_pae, model_pae, y_pae)
            sel_msa, probs_msa, unl_msa_list = select_ids(unlabeled_msa, "MSA", q_msa, model_msa_cls, y_msa)

            for acc in sel_pae:
                idx = int(np.where(ds.ids == acc)[0][0])
                j = unl_pae_list.index(acc)
                p_sel = float(probs_pae[j]) if len(probs_pae) else float("nan")
                ...
                decision_rows.append({
                    ...
                    "oracle_type": "PAE",
                    "accession": acc,
                    "selected_by": ranking_cfg.get("PAE", "uncertainty") if model_pae.trained else fallback_no_model,
            ...
            for acc in sel_msa:
                idx = int(np.where(ds.ids == acc)[0][0])
                j = unl_msa_list.index(acc)
                p_sel = float(probs_msa[j]) if len(probs_msa) else float("nan")
                score = float(abs(p_sel - 0.5))
                decision_rows.append({
                    "policy": policy,
                    "round": int(r+1),
                    "oracle_type": "MSA",
                    "accession": acc,
                    "selected_by": ranking_cfg.get("MSA", "uncertainty") if model_msa_cls.trained else fallback_no_model,
                    "predicted_prob": p_sel,
                    "score": score,
                    "revealed_value": float(ds.msa_depth[idx]),
                    "revealed_label": int(ds.y_msa[idx]),
    ...
    # Eval report summary
    lines = []
    lines.append("# Eval report — Dual‑Oracle Active Acquisition + Joint Gate (v0.4)\n\n")
    ...
    lines.append(f"- usable epsilon fpr: `{eps_fpr}`\n")
    lines.append(f"- prereg_sha256: `{prereg_sha}`\n")
    lines.append(f"- input_metrics_sha256: `{input_hash}`\n\n")

    lines.append("## Policy summary (final)\n\n")
    lines.append("| policy | joint_usable_round | PAE queried | PAE TPR@cap | PAE achieved FPR | PAE covjump? | MSA queried | MSA TPR@cap | MSA achieved FPR | MSA covjump? | final MAE(C3_hat) |\n")
    lines.append("|---|---:|---:|---:|---:|:---:|---:|---:|---:|:---:|---:|\n")
    for policy, ser in series_by_policy.items():
        ...
        lines.append(
            f"| {policy} | {jr} | {pae_q} | {pae_tpr:.6f} | {pae_fpr:.6f} | {'yes' if evp['found'] else 'no'} | "
    ...
    # Plot onepager
    footer = (
        f"subcase=dual_oracle_active_acquisition_v0_4 | parent=afdb_swissprot_tier2p11_confidence_regimes | "
    ...
    plot_onepage_v0_4(out_pdf=run_dir / "tradeoff_onepage.pdf", series_by_policy=series_by_policy, rounds=rounds, meta_footer=footer)

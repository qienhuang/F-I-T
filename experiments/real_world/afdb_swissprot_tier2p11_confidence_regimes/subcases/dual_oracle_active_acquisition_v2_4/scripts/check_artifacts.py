...
def main() -> None:
    ...
    need = {
        "bilevel_tail_q",
        "bilevel_tail_margin_ci_low",
        "bilevel_tail_margin_ci_high",
    ...
    if not need.issubset(set(bi.columns)):
        raise SystemExit("frontier_bilevel_robust_table.csv missing v2.4 tail-gate columns.")

    gate = json.loads((rd / "claims_gate_report.json").read_text(encoding="utf-8"))
    if gate.get("mode") != "bilevel_bootstrap_tail":
        raise SystemExit("claims_gate_report.json mode != bilevel_bootstrap_tail (v2.4 expects tail gate).")
    if gate.get("gate_metric") != "bilevel_tail_margin_ci_low":
        raise SystemExit("claims_gate_report.json gate_metric != bilevel_tail_margin_ci_low.")

    bad = []
    for pol, g in (gate.get("by_policy", {}) or {}).items():
        lo = g.get("bilevel_tail_margin_ci_low", None)
        allowed = bool(g.get("allowed", False))
        if lo is not None and allowed and float(lo) <= 0:
            bad.append(pol)
    if bad:
        raise SystemExit("Claims gate inconsistency (allowed but tail ci_low<=0):\n" + "\n".join(f"- {p}" for p in bad))

    print("OK: v2.4 artifacts + leakage pass + bilevel tail CI gate + claims pack present.")

...
def _fpr_tpr_prec(y_true: np.ndarray, scores: np.ndarray, thr: float) -> Dict[str, float]:
...
def choose_threshold_max_tpr_under_fpr(y_val: np.ndarray, s_val: np.ndarray, fpr_cap: float) -> float:
    ...
    for thr in candidates:
        m = _fpr_tpr_prec(y_val, s_val, float(thr))
        if m["fpr"] <= fpr_cap and m["tpr"] > best_tpr:
            best_tpr = m["tpr"]
            best_thr = float(thr)
    return best_thr

def monitorability_operating_points(
    y_val: np.ndarray,
    s_val: np.ndarray,
    y_test: np.ndarray,
    s_test: np.ndarray,
    ...
    for cap in fpr_targets:
        ...
        m = _fpr_tpr_prec(y_test, s_test, thr_sel)
        ops.append({

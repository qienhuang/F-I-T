...
def _bilevel_bootstrap_stats(
    ...
    """
    Bilevel bootstrap with two statistics:
      - mean margin across policy seeds
      - tail-q margin: quantile_q of margins across policy seeds (operational reliability proxy)

    In each bootstrap replicate:
      1) resample baseline seeds with replacement
      2) resample policy seeds with replacement
      3) recompute baseline upper (q_high) at each policy seed cost under resampled baseline seeds
      4) compute margins for resampled policy seeds: cov - baseline_upper_at_cost
      5) compute mean(margins) and quantile_q(margins)

    Returns CI for both + probabilities that stat > 0.
    """
    ...
    if n_policy == 0 or n_base == 0:
        return {k: float("nan") for k in [
            "mean_margin_mean","mean_margin_ci_low","mean_margin_ci_high","mean_p_advantage",
            "tail_margin_mean","tail_margin_ci_low","tail_margin_ci_high","tail_p_advantage"
        ]}
    ...
    for _ in range(int(n_boot)):
        idx_b = rng.integers(0, n_base, size=n_base)  # baseline resample
        idx_p = rng.integers(0, n_policy, size=n_policy)  # policy resample

        upper = np.quantile(B[idx_b, :], q_high, axis=0)
        margins = cov_vec[idx_p] - upper[idx_p]

        m_mean = float(np.mean(margins))
        m_tail = float(np.quantile(margins, tail_q))

        mean_stats.append(m_mean)
        tail_stats.append(m_tail)
        if m_mean > 0:
            mean_adv += 1
        if m_tail > 0:
            tail_adv += 1
    ...
    tail_stats = np.asarray(tail_stats, dtype=float)

    lo_mean = float(np.quantile(mean_stats, (1-ci)/2))
    hi_mean = float(np.quantile(mean_stats, 1-(1-ci)/2))
    lo_tail = float(np.quantile(tail_stats, (1-ci)/2))
    hi_tail = float(np.quantile(tail_stats, 1-(1-ci)/2))

    return {
        "mean_margin_mean": float(np.mean(mean_stats)),
        "mean_margin_ci_low": lo_mean,
        "mean_margin_ci_high": hi_mean,
        "mean_p_advantage": float(mean_adv / max(1, len(mean_stats))),
        "tail_margin_mean": float(np.mean(tail_stats)),
        "tail_margin_ci_low": lo_tail,
        "tail_margin_ci_high": hi_tail,
        "tail_p_advantage": float(tail_adv / max(1, len(tail_stats))),
    }
...
def main() -> None:
    ...
    tail_q = float(bb.get("tail_quantile", 0.10))
    ...
    # ---------- Frontier tables (policy-only + bilevel mean+tail) ----------
    rows_policy = []
    rows_bi = []
    ...
    for p in policies:
        ...
        # policy-only margins (baseline uncertainty not resampled here; for continuity only)
        ...
        # Gate uses tail CI low > 0 (operational reliability proxy)
        allow_tail = bool(np.isfinite(stats["tail_margin_ci_low"]) and stats["tail_margin_ci_low"] > 0)

        rows_bi.append({
            "policy": p,
            "n_policy_seeds": int(len(covs)),
            "n_baseline_seeds": int(len(baseline_cost_curves)),
            "bilevel_tail_q": float(tail_q),
            ...
            "bilevel_tail_margin_mean": stats["tail_margin_mean"],
            "bilevel_tail_margin_ci_low": stats["tail_margin_ci_low"],
            "bilevel_tail_margin_ci_high": stats["tail_margin_ci_high"],
            "bilevel_tail_p_advantage": stats["tail_p_advantage"],

            "claim_outperforms_random_allowed": allow_tail,
            "gate_metric": "bilevel_tail_margin_ci_low",
        })

    pd.DataFrame(rows_policy).to_csv(main_run / "frontier_policy_robust_table.csv", index=False)

    df_bi = pd.DataFrame(rows_bi).sort_values(by=["claim_outperforms_random_allowed","bilevel_tail_margin_mean"], ascending=[False, False])
    df_bi.to_csv(main_run / "frontier_bilevel_robust_table.csv", index=False)

    gate = {
        "mode": "bilevel_bootstrap_tail",
        "rule": "allow 'outperforms random' only if bilevel_tail_margin_ci_low > 0",
        "gate_metric": "bilevel_tail_margin_ci_low",
        "tail_quantile": float(tail_q),
        "by_policy": {
            row["policy"]: {
                "bilevel_tail_margin_ci_low": float(row["bilevel_tail_margin_ci_low"]),
                "bilevel_tail_margin_ci_high": float(row["bilevel_tail_margin_ci_high"]),
    ...
    # jump robustness across policy seeds (same structure as v2.2/v2.3)
    ...
    write_json(main_run / "bilevel_bootstrap_summary.json", {
        ...
        "tail_quantile": float(tail_q),
        "primary_statistic": str(bb.get("statistic_primary", "tail_margin_q10")),
        "secondary_statistic": str(bb.get("statistic_secondary", "mean_margin")),
        "gate_metric": "bilevel_tail_margin_ci_low",
        "n_baseline_runs": int(len(baseline_cost_curves)),
        "n_policy_runs": int(len(policy_runs)),
    })

    print("v2.4 aggregation complete:")
    for f in [
        "baseline_band.json",
        "policy_band.json",

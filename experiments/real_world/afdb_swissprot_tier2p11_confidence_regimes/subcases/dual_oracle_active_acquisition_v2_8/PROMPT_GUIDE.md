# PROMPT_GUIDE — local CLI assistant (v1.8)
...
7) For any “outperforms random” wording, obey `Claims.md` and the preregistered **two-key gate** in `claims_gate_report.json` (v2.7).
...
- out/<run_id>/claims_gate_report.json
- out/<run_id>/frontier_bilevel_robust_table.csv
- out/<run_id>/Claims.md
Outputs:
...
## v2.8 note — policy-card claims overlay (operational discipline)

After aggregation + claims compilation, each policy card is augmented with a generated section:

- `## Claims & gate status (generated)`

and the machine-readable mapping:

- `policy_cards/claims_overlay.json`

When using an LLM to narrate results, require it to:

1) read `Claims.md` and `claims_overlay.json`,
2) only use phrasing permitted for the policy’s `claim_level`,
3) cite the gate audit numbers verbatim (CI lows) when making any comparative claim.

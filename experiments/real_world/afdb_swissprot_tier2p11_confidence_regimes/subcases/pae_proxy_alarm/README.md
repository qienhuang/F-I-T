# Subcase (canonical) — PAE Proxy Alarm (Non‑LLM Specialist Model)

**Parent case:** `afdb_swissprot_tier2p11_confidence_regimes`  
**Subcase ID:** `pae_proxy_alarm`  
**Pack version:** `v3.0_suite_canonical`  
...
This subcase is canonicalized for the **AFDB non‑LLM small‑models suite**:

- Folder name: `pae_proxy_alarm/`
- `case_id` in prereg is updated accordingly.
- A `PREREG_SMOKE.yaml` + `scripts/ci_check.sh` is added so the subcase can run in CI using **synthetic metrics**.

The estimator logic remains identical to the original v0.1 pack (logistic regression proxy alarm under a strict feature boundary).

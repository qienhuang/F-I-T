...
def extract_logreg_coefficients(model: TrainedModel) -> Optional[Dict[str, float]]:
    # Only works if last step is LogisticRegression
    try:
        clf = model.pipeline.named_steps["clf"]
        coef = clf.coef_.reshape(-1)
        return {name: float(c) for name, c in zip(model.feature_names, coef)}
    except Exception:
        return None

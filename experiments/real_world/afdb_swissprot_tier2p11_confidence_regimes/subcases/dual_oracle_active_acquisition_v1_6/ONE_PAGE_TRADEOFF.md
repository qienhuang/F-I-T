# ONE_PAGE_TRADEOFF — Dual‑Oracle + Δ‑Lag + Leakage Audit (v1.6)
...
## Panel D addendum — joint coverage jump marker

- In Panel D, add an extra marker per policy for `E_covjump_joint.round_index` (if found).
- Interpretation:
  - if the marker coincides with `joint usable`, the system’s “coverage jump” is largely an availability/monitorability transition.
  - if it happens later, the system is jointly usable earlier but only achieves meaningful joint coverage later (a different regime story).

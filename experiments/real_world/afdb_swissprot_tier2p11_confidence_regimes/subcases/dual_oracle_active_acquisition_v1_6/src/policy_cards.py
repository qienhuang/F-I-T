...
def _plot_joint_cov(out_png: Path, policy: str, cov_joint: List[float], r_covjump: int | None) -> None:
    ...
    plt.title(f"{policy}: joint coverage timeline")
...
def write_policy_cards(
    ...
    for _, row in policy_table.iterrows():
        ...
        joint_png = assets_dir / f"{policy}_joint_cov.png"
        ...
        _plot_joint_cov(joint_png, policy, cov_joint=(ser.get("joint") or {}).get("cov_joint_usable", []), r_covjump=int(row.get("r_covjump_joint")) if row.get("r_covjump_joint") is not None else None)

        for p in [learning_png, alloc_png, sweep_png, floor_png, joint_png]:
        ...
        text.append(f"![Joint coverage timeline](assets/{joint_png.name})\n\n")
    ...
    lines.append("| policy | r_joint_usable | r_covjump_joint | delta_lag | leakage_pass | card |\n")
    lines.append("|---|---:|---:|---:|---|---|\n")
    for _, r in idx.iterrows():
        policy = r["policy"]
        lines.append(
            f"| `{policy}` | {int(r['r_joint_usable'])} | {int(r.get('r_covjump_joint', -1))} | {int(r['delta_lag'])} | {bool(r['leakage_pass'])} | [{policy}]({r['card_path']}) |\n"
        )

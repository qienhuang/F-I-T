...
def main() -> None:
    ...
    for ps, ser in series_by_policy.items():
        ...
        cov_final = float(ser.get("joint", {}).get("cov_joint_usable", [float("nan")])[-1]) if ser.get("joint", {}).get("cov_joint_usable") else float("nan")
    ...
    policy_df = pd.DataFrame(rows).sort_values(by=["r_joint_usable", "delta_lag", "policy"])
    ...
    lines: List[str] = []
    lines.append("# Eval report — Dual‑Oracle + Δ‑Lag + Joint Coverage Jump (v1.6)\n\n")
    ...
    footer = (
        f"subcase=dual_oracle_active_acquisition_v1_6 | parent=afdb_swissprot_tier2p11_confidence_regimes | "

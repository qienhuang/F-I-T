...
from .plot_onepager import plot_onepage_v0_6
...
def main() -> None:
    ...
    # Holdout/pool per oracle availability
    ...
    # initial stable orders for each oracle pool
    ...
    for spec in policy_specs:
        ...
        # log init labels as round 0 reveals
        ...
        for r in range(0, rounds + 1):
            # Build labeled indices per oracle
            ...
            pae_ok = bool((op_pae_primary["fpr"] <= (primary_fpr + eps_fpr)) and (op_pae_primary["tpr"] >= tpr_min))

            # MSA classifier + regressor
            ...
            msa_ok = bool((op_msa_primary["fpr"] <= (primary_fpr + eps_fpr)) and (op_msa_primary["tpr"] >= tpr_min))
            ...
            round_metrics[spec].append({
                "round": int(r),
                "pae": {"queried": int(pae_used), "tpr_primary": float(op_pae_primary["tpr"]), "fpr_primary": float(op_pae_primary["fpr"]), "usable": bool(pae_ok)},
                "msa_cls": {"queried": int(msa_used), "tpr_primary": float(op_msa_primary["tpr"]), "fpr_primary": float(op_msa_primary["fpr"]), "usable": bool(msa_ok)},
            ...
            if alloc_policy == "fixed_split":
                ...
                # compute uncertainty mass on unlabeled pools (cheap)
                if len(unl_pae_list):
            ...
            def select_for_oracle(oracle: str, q: int, unl_set: set[str], model, y_lab: np.ndarray, K: int) -> Tuple[List[str], List[Dict[str, float]]]:
                ...
                if (not model.trained) and fallback_no_model == "random_hash":
                    ...
                    pick_idx = order[:q]
                    return [unl_list[i] for i in pick_idx], [{"predicted_prob": float(probs[i])} for i in pick_idx]
                ...
                if rank_policy == "composite_batch_ff":
                    # build ref_z from labeled set in standardized feature space
                    if oracle == "PAE":
                    ...
                    pick_idx = res.selected_indices
                    per = res.per_item
                    picked = [unl_list[i] for i in pick_idx]
                    return picked, per
    ...
    # Eval report summary
    lines = []
    lines.append("# Eval report — Dual‑Oracle + Batch Diversity (v0.6)\n\n")
    ...
    lines.append("- Allowed claims are budgeted learning curves, joint gate outcome, and proxy regression quality under the locked boundary.\n")
    ...
    # Plot onepager
    footer = (
        f"subcase=dual_oracle_active_acquisition_v0_6 | parent=afdb_swissprot_tier2p11_confidence_regimes | "
    ...
    plot_onepage_v0_6(out_pdf=run_dir / "tradeoff_onepage.pdf", series_by_policy=series_by_policy, rounds=rounds, meta_footer=footer)

# REPRO_CHECKLIST — Dual‑Oracle + Batch Diversity (v0.6)
...
- `decision_trace.csv` includes, for each query:
  - `oracle_type`, `selected_by`, `predicted_prob`, `uncertainty_norm`,
    `novelty`, `novelty_norm`, `composite_score`
- budgets respected per oracle (or early stop due to pool exhaustion)

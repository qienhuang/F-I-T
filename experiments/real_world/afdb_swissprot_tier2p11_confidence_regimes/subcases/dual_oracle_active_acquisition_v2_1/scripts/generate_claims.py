...
def main() -> None:
    ...
    # Prefer policy-robust table if present (forward compatible with v2.2)
    ...
    templates = {
        "allowed_phrase": "Under the preregistered protocol and cost-aware low-FPR cap, {policy} robustly exceeds the random baseline band (margin CI strictly positive).",
        "forbidden_phrase": "Under the preregistered protocol, {policy} outperforms random.",
        "safe_phrase": "Under the preregistered protocol and cost-aware low-FPR cap, {policy} does not show a statistically robust advantage over the random baseline band (margin CI crosses zero or is undefined).",
    ...
    lines.append("- If `dominance_margin_ci_low <= 0`, the phrase **\"outperforms random\"** is forbidden.\n")

...
def main() -> None:
    ...
    # policy card assets manifest should include the cost-based joint-coverage plot
    ...
    # Frontier table must include Pareto flags + baseline/headroom columns
    ft = pd.read_csv(rd / "frontier_table.csv")
    need_cols = {
    ...
    if not need_cols.issubset(set(ft.columns)):
        raise SystemExit("frontier_table.csv missing required frontier columns (audit failure).")

    # Policy cards must embed jump-type sensitivity (audit that at least one card contains the section header)
    card_dir = rd / "policy_cards"
    md_files = list(card_dir.glob("*.md"))
    if not md_files:
        raise SystemExit("No policy cards found (audit failure).")
    has_sens = False
    for p in md_files[:20]:
        t = p.read_text(encoding="utf-8", errors="ignore")
        if "Jump-type sensitivity (preregistered grid)" in t:
            has_sens = True
            break
    if not has_sens:
        raise SystemExit("Policy cards missing jump-type sensitivity section (audit failure).")

    print("OK: artifacts present + leakage_audit pass + Δ-lag audit pass + covjump audit pass + cost-asset audit pass + frontier audits pass + card sensitivity audit pass.")

...
def plot_onepage_v1_1(
    ...
    for policy, ser in series_by_policy.items():
        ...
        items.append((policy, r_pae, r_msa, r_floor, r_joint))
    items.sort(key=lambda x: (x[4], x[0]))  # by joint usable, then name
    ...
    axD.set_title("(D) Δ-lag: floor-clear → joint-usable")

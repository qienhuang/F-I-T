...
def evaluate_holdout(y_val: np.ndarray, s_val: np.ndarray, y_test: np.ndarray, s_test: np.ndarray, fpr_targets: List[float]) -> Dict[str, object]:
    out: Dict[str, object] = {}
    out["roc_auc"] = float(roc_auc_score(y_test, s_test)) if len(np.unique(y_test)) > 1 else float("nan")
    out["pr_auc"] = float(average_precision_score(y_test, s_test)) if len(np.unique(y_test)) > 1 else float("nan")

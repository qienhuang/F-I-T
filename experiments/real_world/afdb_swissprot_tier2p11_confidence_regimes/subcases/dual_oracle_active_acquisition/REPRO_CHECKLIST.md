# REPRO_CHECKLIST — Dual‑Oracle + Δ‑Lag + Leakage Audit (v1.8)
...
## Added in v1.8 — joint coverage jump checks
...
## 8) Robustness gate evaluation (v2.7 two-key gate)

This subcase uses a **bilevel robustness gate** that requires both:

- CVaR/expected shortfall gate CI low > 0, and
- tail-min audit CI low > 0.

To reproduce the robust tables (recommended), run the smoke/CI script or do it manually.

### Option A: one-shot smoke/CI

```bash
bash scripts/ci_check.sh
```

### Option B: manual (baseline grid + policy grid + aggregate)

```bash
python scripts/run_baseline_grid.py --prereg PREREG.yaml --out_root out_baseline
python scripts/run_policy_grid.py --prereg PREREG.yaml --out_root out_policy
python -m src.run --prereg PREREG.yaml --run_id MAIN
python scripts/aggregate_v2_7.py --main_run out/MAIN --baseline_root out_baseline --policy_root out_policy
python scripts/generate_claims.py --run_dir out/MAIN
python scripts/check_artifacts.py --run_dir out/MAIN
```

Outputs:

- `out/MAIN/frontier_bilevel_robust_table.csv`
- `out/MAIN/claims_gate_report.json`
- `out/MAIN/Claims.md` (graded claims; “outperforms random” allowed only for `claim_level=strong`)

- [ ] `policy_cards/claims_overlay.json` exists and each policy card includes the claims overlay section.

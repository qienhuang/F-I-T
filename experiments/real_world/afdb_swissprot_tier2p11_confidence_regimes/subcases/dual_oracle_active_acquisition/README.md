# Subcase (canonical) — Dual‑Oracle Active Acquisition (Two‑Key Gate + Policy‑Card Claims Overlay)

**Parent case:** `afdb_swissprot_tier2p11_confidence_regimes`  
**Subcase ID:** `dual_oracle_active_acquisition`  
**Pack version:** `v2.9_repo_ready_canonical`  
**Date locked:** 2026‑01‑25

**Canonical release notes:** see `RELEASE_NOTES.md`  
...
## v2.9 addition — canonical subcase name + release notes (repo convergence)

v2.9 is a **repo convergence** release:

- The subcase directory is now **canonical**: `dual_oracle_active_acquisition/` (no version suffix).
- v2.8 functionality is preserved: **two-key robustness gate + graded claims + policy-card claims overlay**.
- CI scripts and claim-pack headers are updated to `v2.9` to avoid stale “v2.8” semantics.
- A `RELEASE_NOTES.md` is included to document the evolution from v2.0 → v2.8 and how v2.9 canonicalizes the subcase without changing the estimator/boundary logic.

Engineering intent:
- Keep **one** canonical subcase in the repo.
- Preserve historical evolution via `RELEASE_NOTES.md` + git tags/releases, rather than proliferating subcase directories.

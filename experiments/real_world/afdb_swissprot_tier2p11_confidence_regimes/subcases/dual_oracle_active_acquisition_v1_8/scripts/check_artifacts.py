...
def main() -> None:
    ...
    # Frontier table must include Pareto flags
    ft = pd.read_csv(rd / "frontier_table.csv")
    need_cols = {"pareto_cost_to_joint_vs_cov", "pareto_cost_to_covjump_vs_cov"}
    if not need_cols.issubset(set(ft.columns)):
        raise SystemExit("frontier_table.csv missing Pareto flag columns (audit failure).")

    print("OK: artifacts present + leakage_audit pass + Δ-lag audit pass + covjump audit pass + cost-asset audit pass + frontier audits pass.")

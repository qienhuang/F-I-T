...
def main() -> None:
    ...
    fr = pd.read_csv(rd / "frontier_policy_robust_table.csv")
    ...
    if not need.issubset(set(fr.columns)):
        raise SystemExit("frontier_policy_robust_table.csv missing CI gate columns.")
    ...
    # gate consistency: each allowed must correspond to ci_low > 0 if present
    bad = []
    for pol, g in (gate.get("by_policy", {}) or {}).items():
        lo = g.get("dominance_margin_ci_low", None)
    ...
    print("OK: v2.2 artifacts + leakage pass + CI gate + manifest + claims pack present.")

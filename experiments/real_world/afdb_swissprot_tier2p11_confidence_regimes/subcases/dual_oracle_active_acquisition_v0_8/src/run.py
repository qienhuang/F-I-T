...
from .plot_onepager import plot_onepage_v0_8
...
def ranking_policy_to_candidate_basis(rank_policy: str) -> str:
    if rank_policy == "composite_batch_ff_uK":
        return "uncertainty"
    if rank_policy == "composite_batch_ff_rK":
        return "random_hash"
    # default (for legacy / safety)
...
def main() -> None:
    ...
    for spec in policy_specs:
        ...
        for acc in sorted(list(labeled_pae)):
            ...
            decision_rows.append({
                ...
                "allocation_policy": alloc_policy, "ranking_policy": rank_policy,
        ...
        pae_series = {"queried_labels": [], "tpr_primary": [], "fpr_primary": []}
        msa_cls_series = {"queried_labels": [], "tpr_primary": [], "fpr_primary": []}
        ...
        for r in range(0, rounds + 1):
            ...
            # MSA regressor (proxy channel) — filter finite y & aligned X (defensive)
            ...
            def select_for_oracle(oracle: str, q: int, unl_set: set[str], model, y_lab: np.ndarray, K: int) -> Tuple[List[str], List[Dict[str, float]], str]:
                ...
                if rank_policy in ("composite_batch_ff_uK", "composite_batch_ff_rK"):
                    basis = ranking_policy_to_candidate_basis(rank_policy)
                    if oracle == "PAE":
                    ...
                    return picked, res.per_item, basis

                raise ValueError(f"Unknown ranking policy: {rank_policy}")

            sel_pae, per_pae, basis_pae = select_for_oracle("PAE", q_pae, unlabeled_pae, model_pae, y_pae, K=K_pae)
            sel_msa, per_msa, basis_msa = select_for_oracle("MSA", q_msa, unlabeled_msa, model_msa_cls, y_msa, K=K_msa)
    ...
    # Eval report summary
    lines = []
    lines.append("# Eval report — Dual‑Oracle + Candidate‑Pool Ablation (v0.8)\n\n")
    ...
    lines.append("| policy | alloc | rank | E_joint_usable_round | E_covjump_pae_round | E_covjump_msa_round |\n")
    lines.append("|---|---|---|---:|---:|---:|\n")
    for spec, ser in series_by_policy.items():
        ...
        lines.append(f"| {spec} | {ser['allocation_policy']} | {ser['ranking_policy']} | {jr} | {cr_pae} | {cr_msa} |\n")
    lines.append("\n")

    lines.append("## Final operating‑point summary (holdout @ primary cap)\n\n")
    lines.append("| policy | PAE queried | PAE TPR@cap | PAE FPR | MSA queried | MSA TPR@cap | MSA FPR | final MAE(C3_hat) |\n")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|\n")
    for spec, ser in series_by_policy.items():
        ...
        lines.append(
            f"| {spec} | {pae_q} | {pae_tpr:.6f} | {pae_fpr:.6f} | {msa_q} | {msa_tpr:.6f} | {msa_fpr:.6f} | {mae_final:.6f} |\n"
        )
    lines.append("\n")

    lines.append("## Interpretation rule\n\n")
    lines.append("- Policy decisions were driven by labeled validation diagnostics only (no holdout leakage).\n")
    lines.append("- Candidate pool ablation is valid only for `*_uK` vs `*_rK` pairs under the same allocation policy.\n")
    lines.append("- Event claims must reference `event_summary.json`.\n\n")
    _write_text(run_dir / "eval_report.md", "".join(lines))

    footer = (
        f"subcase=dual_oracle_active_acquisition_v0_8 | parent=afdb_swissprot_tier2p11_confidence_regimes | "
    ...
    plot_onepage_v0_8(out_pdf=run_dir / "tradeoff_onepage.pdf", series_by_policy=series_by_policy, meta_footer=footer)

# ONE_PAGE_TRADEOFF — Dual‑Oracle + Alpha/K Ablations (v0.9)
...
## Panel D — Joint gate frontier (event‑aware)

Define `E_joint_usable_round(policy)`:
...
- X axis: `E_joint_usable_round` (lower is better; if never achieved, set to `rounds+1`)

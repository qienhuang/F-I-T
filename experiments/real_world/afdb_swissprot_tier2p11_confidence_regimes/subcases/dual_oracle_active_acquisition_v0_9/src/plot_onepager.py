...
def plot_onepage_v0_9(
    ...
    for policy in policies:
        ev = series_by_policy[policy]["events"]["E_joint_usable"]
        jr = int(ev["round_index"]) if ev["found"] else int(series_by_policy[policy]["joint"]["joint_usable_round"])
    ...
    for i, policy in enumerate(policies):
        axD.text(x[i], y[i], policy, fontsize=6)

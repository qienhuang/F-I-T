...
def fpr_floor_at_tpr_min(y_true: np.ndarray, scores: np.ndarray, tpr_min: float) -> float:
    if len(y_true) == 0:
        return 1.0
    if len(np.unique(y_true)) < 2:
        # floor is undefined/degenerate; treat as worst-case for hardness
        return 1.0
    fpr, tpr, _ = roc_curve(y_true, scores)
    mask = tpr >= float(tpr_min)
    if not np.any(mask):
        return 1.0
...
def evaluate_binary_classifier(
    ...
    out["fpr_floor_holdout_at_tpr_min"] = fpr_floor_at_tpr_min(y_test, s_test, tpr_min=tpr_min_for_floor)
    out["fpr_floor_val_at_tpr_min"] = fpr_floor_at_tpr_min(y_val, s_val, tpr_min=tpr_min_for_floor)
    return out

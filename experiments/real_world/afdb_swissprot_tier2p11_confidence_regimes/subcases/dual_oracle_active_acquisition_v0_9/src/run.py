...
from .plot_onepager import plot_onepage_v0_9
...
def main() -> None:
    ...
    event_summary: Dict[str, Any] = {"events_by_policy": {}, "W_jump": W_jump, "delta_tpr": delta_tpr, "primary_cap": primary_fpr}
    ...
    for spec in policy_specs:
        ...
        pae_series = {"queried_labels": [], "tpr_primary": [], "fpr_primary": [], "fpr_floor_at_tpr_min": []}
        msa_cls_series = {"queried_labels": [], "tpr_primary": [], "fpr_primary": [], "fpr_floor_at_tpr_min": []}
    ...
    # Eval report summary
    lines = []
    lines.append("# Eval report — Dual‑Oracle + Alpha/K Ablations (v0.9)\n\n")
    ...
    lines.append("| policy | alloc | base | alpha | K | E_joint_usable_round | E_covjump_pae_round | E_covjump_msa_round |\n")
    lines.append("|---|---|---|---:|---:|---:|---:|---:|\n")
    for ps, ser in series_by_policy.items():
        ev = ser["events"]
        jr = int(ev["E_joint_usable"]["round_index"])
        cr_pae = int(ev["E_covjump_pae"]["round_index"]) if ev["E_covjump_pae"]["found"] else (rounds + 1)
        cr_msa = int(ev["E_covjump_msa"]["round_index"]) if ev["E_covjump_msa"]["found"] else (rounds + 1)
        lines.append(f"| {ps} | {ser['allocation_policy']} | {ser['ranking_base']} | {ser['alpha_used']:.2f} | {ser['K_used']} | {jr} | {cr_pae} | {cr_msa} |\n")
    lines.append("\n")
    ...
    lines.append("- Alpha/K comparisons are valid only among policies that share allocation and base ranking.\n")
    ...
    footer = (
        f"subcase=dual_oracle_active_acquisition_v0_9 | parent=afdb_swissprot_tier2p11_confidence_regimes | "
    ...
    plot_onepage_v0_9(out_pdf=run_dir / "tradeoff_onepage.pdf", series_by_policy=series_by_policy, meta_footer=footer)

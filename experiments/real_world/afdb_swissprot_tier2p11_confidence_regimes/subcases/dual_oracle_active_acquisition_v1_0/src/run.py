...
from .plot_onepager import plot_onepage_v1_0
...
def main() -> None:
    ...
    for spec in policy_specs:
        ...
        summary["by_policy"][spec] = {
            ...
            "durations": dur,
            "events": {
                "E_floor_resolved_pae": ev["E_floor_resolved_pae"],
                "E_floor_resolved_msa": ev["E_floor_resolved_msa"],
                "E_joint_usable": ev["E_joint_usable"],
                "E_covjump_pae": ev["E_covjump_pae"],
                "E_covjump_msa": ev["E_covjump_msa"],
            },
        }
    ...
    # Eval report summary
    lines = []
    lines.append("# Eval report — Dual‑Oracle + Regime Diagnostics (v1.0)\n\n")
    ...
    lines.append("| policy | alloc | base | alpha | K | floor_pae | floor_msa | joint | covjump_pae | covjump_msa |\n")
    lines.append("|---|---|---|---:|---:|---:|---:|---:|---:|---:|\n")
    for ps, ser in series_by_policy.items():
        ...
        crp = int(ev["E_covjump_pae"]["round_index"]) if ev["E_covjump_pae"]["found"] else (rounds + 1)
        crm = int(ev["E_covjump_msa"]["round_index"]) if ev["E_covjump_msa"]["found"] else (rounds + 1)
        lines.append(f"| {ps} | {ser['allocation_policy']} | {ser['ranking_base']} | {ser['alpha_used']:.2f} | {ser['K_used']} | {frp} | {frm} | {jr} | {crp} | {crm} |\n")
    lines.append("\n")
    ...
    lines.append("- Alpha/K comparisons are valid only among policies that share allocation + ranking base.\n")
    lines.append("- `FPR_floor@TPRmin` is a hardness metric: if it exceeds cap, the alarm is unusable at that cap.\n")
    lines.append("- `E_floor_resolved_*` are phase‑like markers: they must precede (or equal) joint usability.\n\n")
    _write_text(run_dir / "eval_report.md", "".join(lines))

    footer = (
        f"subcase=dual_oracle_active_acquisition_v1_0 | parent=afdb_swissprot_tier2p11_confidence_regimes | "
    ...
    plot_onepage_v1_0(out_pdf=run_dir / "tradeoff_onepage.pdf", series_by_policy=series_by_policy, meta_footer=footer, rounds=rounds)

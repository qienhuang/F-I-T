...
def plot_onepage_v1_0(
    ...
    # Panel D: regime event markers (sorted by earliest joint usability)
    ...
    for policy, ser in series_by_policy.items():
        ...
        items.append((policy, r_pae, r_msa, r_joint))
    items.sort(key=lambda x: (x[3], x[0]))  # by joint usable, then name

    y_pos = np.arange(len(items))
    x_pae = [it[1] for it in items]
    x_msa = [it[2] for it in items]
    x_joint = [it[3] for it in items]
    ...
    axD.set_title("(D) Regime event markers (floor resolution vs joint usability)")

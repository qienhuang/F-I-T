"""
Run the NYC TLC Tier-2 P11 pipeline end-to-end (CPU-friendly).
...
It is intentionally simple and deterministic: it does not change the prereg in-place.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run TLC pipeline end-to-end")
    ...
    # If the user overrides raw_glob, write a temporary prereg file (do not mutate the original).
    tmp_prereg_path: Path | None = None
    prereg_for_run = prereg_path
    if args.raw_glob:
        prereg = yaml.safe_load(prereg_path.read_text(encoding="utf-8"))
        prereg.setdefault("boundary", {})["raw_glob"] = args.raw_glob
        fd, tmp_name = tempfile.mkstemp(prefix="tlc_prereg_", suffix=".yaml")
        os.close(fd)
    ...
    try:
        # Default layout follows the module defaults; out_root lets you redirect everything into a folder.
        if args.out_root:
            out_root = (workdir / args.out_root).resolve()
            cleaned = out_root / "data" / "cleaned" / "daily_state.parquet"
            ...
            cleaned = workdir / "data" / "cleaned" / "daily_state.parquet"

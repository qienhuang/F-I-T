...
> **Status: OK_PER_YEAR** - Coherence passes within yearly windows.
> 
> Interpretation rule: interpret signatures within each year; pooled level shifts can break pooled coherence.

### Coherence Test Results

- `C_congestion` vs `C_price_pressure`: rho = 0.543 (n=1826) **[FAIL]**

### Yearly Coherence (Diagnostic)

| Year | Status | Pair | rho | n | Pass |
|------|--------|------|-----|---|------|
| 2019 | OK | `C_congestion` vs `C_price_pressure` | 0.954 | 365 | PASS |
| 2020 | OK | `C_congestion` vs `C_price_pressure` | 0.964 | 366 | PASS |
| 2021 | OK | `C_congestion` vs `C_price_pressure` | 0.929 | 365 | PASS |
| 2022 | OK | `C_congestion` vs `C_price_pressure` | 0.863 | 365 | PASS |
| 2023 | OK | `C_congestion` vs `C_price_pressure` | 0.966 | 365 | PASS |
...
Coherence passes **within yearly windows**, but may fail when pooling years (level shifts).

**Interpretation rule:** treat signatures as interpretable **within each year**.
Pooled change points are still reported, but should be treated as diagnostic unless they also appear consistently within-year.

Recommended practice: run year-by-year change-point detection and report per-year tables alongside any pooled plot.
